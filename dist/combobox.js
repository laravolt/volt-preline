...
function defaultValueKey(option, displayValue) {
    return typeof option === 'object' && option !== null ? (displayValue(option) ?? '') : String(option);
}
export function Combobox(handle) {
    let keyOf = (option) => handle.props.valueKey?.(option) ?? defaultValueKey(option, handle.props.displayValue);
    let labelOf = (option) => handle.props.displayValue(option) ?? '';
    let keyOfNullable = (option) => (option == null ? null : keyOf(option));
    let query = '';
    let currentKey = keyOfNullable(handle.props.value ?? handle.props.defaultValue);
    ...
    let context = {
        ...
        renderOption(option) {
            return handle.props.children(option);
        },
        ...
        setQuery(next) {
            ...
            for (let key of searchValues.keys())
                if (!keys.has(key))
                    searchValues.delete(key);
            void context.updateOptions?.();
        },
        updateOptions: null,
        input: null,
    };
    function passesFilter(option) {
        if (query === '')
            return true;
    ...
    function syncSearchValue(values, option) {
    ...
    return () => {
        ...
        let { options: _options, displayValue: _displayValue, filter: _filter, valueKey: _valueKey, value: _value, defaultValue: _defaultValue, onChange: _onChange, anchor: _anchor, id: _id, name: _name, placeholder: _placeholder, autoFocus: _autoFocus, disabled: _disabled, invalid: _invalid, 'aria-describedby': _ariaDescribedBy, children: _children, mix: _mix, ...inputProps } = rest;
        ...
        return (_jsx(combobox.Context, { name: name, defaultValue: currentKey, disabled: disabled, children: _jsxs("span", { "data-slot": "control", className: cx(className, 'relative block w-full'), children: [_jsx(ComboboxInput, { ...inputProps, ...(initialLabel !== null ? { defaultValue: initialLabel } : {}), id: id, autoFocus: autoFocus, disabled: disabled, placeholder: placeholder, "aria-describedby": ariaDescribedBy, "aria-invalid": invalid ? 'true' : undefined, "data-invalid": invalid ? '' : undefined, mix: mix, onCommit: (key) => {
                            if (key === currentKey)
                                return;
                            currentKey = key;
                        ...
                        'block w-full rounded-lg py-2.5 ps-4 pe-9 sm:py-3', 'text-base text-layer-foreground placeholder:text-muted-foreground sm:text-sm', 'border-layer-line bg-layer shadow-2xs', 
                        ...
                        // Disabled
                        'disabled:pointer-events-none disabled:opacity-50') }), _jsx(ComboboxButton, { disabled: disabled }), _jsx(ComboboxOptions, {}), name && _jsx("input", { mix: combobox.hiddenInput() })] }) }, contextKey));
    };
}
function ComboboxInput(handle) {
    let listboxCtx = handle.context.get(listbox.Context);
    let outer = handle.context.get(Combobox);
    return () => {
        let { onCommit, mix, ...props } = handle.props;
        return (_jsx("input", { ...props, mix: [
...
function ComboboxButton(handle) {
    let ctx = handle.context.get(combobox.Context);
    let outer = handle.context.get(Combobox);
    return () => (_jsx("button", { type: "button", tabIndex: -1, "aria-haspopup": "listbox", "aria-expanded": ctx.isOpen ? 'true' : 'false', "aria-controls": ctx.listId, disabled: handle.props.disabled, className: "group absolute inset-y-0 end-0 flex items-center px-3 text-muted-foreground hover:text-foreground disabled:pointer-events-none", mix: [
        ...
        ], children: _jsxs("svg", { className: "size-3.5 shrink-0", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round", "aria-hidden": "true", children: [_jsx("path", { d: "m7 15 5 5 5-5" }), _jsx("path", { d: "m7 9 5-5 5 5" })] }) }));
}
/**
 ...
 */
function ComboboxOptions(handle) {
    ...
    return () => (_jsx("div", { mix: [
        ...
        'z-50 max-h-72 space-y-0.5 overflow-hidden overflow-y-auto rounded-lg border border-select-line bg-select p-1 shadow-xl', '[&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:rounded-none [&::-webkit-scrollbar-track]:bg-scrollbar-track [&::-webkit-scrollbar-thumb]:bg-scrollbar-thumb', 'select-none focus:outline-hidden empty:invisible', 
        // Transitions (native popover: fade in via @starting-style, fade out via discrete display/overlay transition)
        'opacity-0 transition-[opacity,display,overlay] transition-discrete duration-100 ease-in open:opacity-100 starting:open:opacity-0 not-open:pointer-events-none'), children: outer.options.map((option) => (_jsx(Fragment, { children: outer.renderOption(option) }, outer.keyOf(option)))) }));
}
export function ComboboxOption(handle) {
    ...
    return () => {
        ...
        return (_jsxs("div", { ...divProps, mix: [
            ...
            // Disabled
            'aria-disabled:pointer-events-none aria-disabled:opacity-50'), children: [_jsx("span", { className: cx(className, optionContentClasses), children: children }), _jsx("svg", { className: "hidden size-3.5 shrink-0 text-primary group-aria-selected/option:block", xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round", "aria-hidden": "true", children: _jsx("polyline", { points: "20 6 9 17 4 12" }) })] }));
    };
}
export function ComboboxLabel(handle) {
    return () => {
        let { className, rest } = splitProps(handle.props);
...
export function ComboboxDescription(handle) {
    return () => {
        let { className, rest } = splitProps(handle.props);
        let { children, ...props } = rest;
