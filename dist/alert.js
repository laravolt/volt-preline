...
export function Alert(handle) {
    ...
    return () => {
        ...
        let { size = 'md', align = 'start', open: _open, onClose: _onClose, 'aria-labelledby': _labelledBy, 'aria-describedby': _describedBy, children, ...dialogProps } = rest;
        ids.align = align;
        modal.afterRender();
        return (_jsx("dialog", { ...dialogProps, "data-slot": "alert", role: "alertdialog", "aria-modal": "true", mix: modal.dialog(), className: dialogHostClasses, children: _jsx("div", { className: "flex min-h-full items-center justify-center p-3 sm:p-4", children: _jsx("div", { "data-slot": "alert-panel", mix: modal.panel, className: cx(className, 'w-full', sizes[size], align === 'center' ? 'text-center' : 'text-start', 'flex flex-col rounded-xl border border-overlay-line bg-overlay p-4 shadow-2xs sm:p-5', 'scale-95 opacity-0 transition-all transition-discrete duration-300 ease-out', 'group-open:scale-100 group-open:opacity-100 starting:group-open:scale-95 starting:group-open:opacity-0'), children: children }) }) }));
    };
}
...
export function AlertActions(handle) {
    ...
    return () => {
        ...
        return (_jsx("div", { ...props, className: cx(className, 'mt-5 flex flex-col-reverse items-center gap-2 *:w-full', align === 'center'
                ? 'sm:flex-row sm:justify-center sm:*:w-auto'
                : 'sm:flex-row sm:justify-end sm:*:w-auto'), children: children }));
    };
}
