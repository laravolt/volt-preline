...
// --- modal controller -------------------------------------------------------------------------
export function createModal(handle, ids, read) {
    let dialog = null;
    let panel = null;
    let restoreTarget = null;
    let releaseScrollLock = () => { };
    let pressStartedOutside = false;
    let isOutside = (target) => !(panel && target instanceof Node && panel.contains(target));
    ...
    function syncAria(el) {
        ...
        let has = (id) => !!el.querySelector(`#${CSS.escape(id)}`);
    ...
    function restoreFocus() {
        ...
        if (focusIsLost && restoreTarget?.isConnected)
            restoreTarget.focus();
        restoreTarget = null;
    }
    return {
        ...
        dialog() {
            return [
                ...
                on('click', (event) => {
                    if (pressStartedOutside && isOutside(event.target))
                        requestClose();
                }),
            ];
        },
