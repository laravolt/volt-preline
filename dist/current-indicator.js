...
export class LayoutGroup {
    ...
    snapshot() {
        if (this.node?.isConnected)
            this.lastBox = this.node.getBoundingClientRect();
    }
}
function playHandoff(node, from) {
...
export function CurrentIndicator(handle) {
    return () => {
        ...
        group?.snapshot();
        return (_jsx("span", { "aria-hidden": "true", "data-slot": "current-indicator", className: className, mix: [
                animateLayout({ ...spring('snappy') }),
                ref((el, signal) => {
                    if (!group)
                        return;
                    let node = el;
                    if (group.node !== node && group.lastBox)
                        playHandoff(node, group.lastBox);
                    group.node = node;
                    group.lastBox = null;
