...
export function SwitchGroup(handle) {
    return () => {
...
export function SwitchField(handle) {
    return () => {
        let { className, rest } = splitProps(handle.props);
        ...
        return (_jsx("div", { "data-slot": "field", "data-disabled": disabled ? '' : undefined, ...attrs, className: cx(className, 'grid grid-cols-[1fr_auto] items-start gap-x-6 gap-y-0.5', '[&>[data-slot=control]]:col-start-2 [&>[data-slot=control]]:row-start-1', '[&>[data-slot=label]]:col-start-1 [&>[data-slot=label]]:row-start-1 [&>[data-slot=label]]:leading-6', '[&>[data-slot=description]]:col-start-1 [&>[data-slot=description]]:row-start-2', '[&>[data-slot=error]]:col-start-1', 'has-[[data-slot=description]]:[&_[data-slot=label]]:font-medium'), children: children }));
    };
}
...
export function Switch(handle) {
    return () => {
        ...
        return (_jsxs("span", { "data-slot": "control", className: cx(className, 'relative inline-block h-6 w-11 shrink-0 align-middle'), children: [_jsx("input", { ...attrs, type: "checkbox", role: "switch", id: id, ...(checked !== undefined ? { checked } : {}), ...(defaultChecked !== undefined ? { defaultChecked } : {}), disabled: disabled || undefined, "aria-describedby": describedBy, className: "peer absolute inset-0 z-10 m-0 size-full cursor-pointer appearance-none rounded-full border-0 bg-transparent opacity-0 shadow-none checked:bg-transparent focus:ring-0 focus:ring-offset-0 disabled:cursor-not-allowed", mix: on('change', (event) => {
                        let target = event.currentTarget;
                        onChange?.(target.checked, event);
                    }) }), _jsx("span", { "aria-hidden": "true", className: cx('absolute inset-0 rounded-full bg-surface-1 transition-colors duration-200 ease-in-out', 'peer-focus-visible:ring-2 peer-focus-visible:ring-primary peer-focus-visible:ring-offset-2 peer-focus-visible:ring-offset-background', 'peer-disabled:pointer-events-none peer-disabled:opacity-50', colors[color] ?? colors['dark/zinc']) }), _jsx("span", { "aria-hidden": "true", className: cx('absolute start-0.5 top-1/2 size-5 -translate-y-1/2 rounded-full bg-switch shadow-sm transition-transform duration-200 ease-in-out', 'peer-checked:translate-x-full peer-disabled:pointer-events-none peer-disabled:opacity-50', color === 'white' && 'ring-1 ring-line-3') })] }));
    };
}
