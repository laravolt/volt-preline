...
export type ListboxLabelProps = { className?: string; class?: string; children?: RemixNode } & ElementProps

export type ListboxDescriptionProps = { className?: string; class?: string; children?: RemixNode } & ElementProps

/** Text content of a Remix node tree (used for typeahead labels and the select's default label). */
export function nodeText(node: RemixNode): string {
  return collectText(node).replace(/\s+/g, ' ').trim()
}

function collectText(node: RemixNode): string {
  if (node == null || typeof node === 'boolean') return ''
  if (typeof node === 'string' || typeof node === 'number' || typeof node === 'bigint') return String(node)
  if (Array.isArray(node)) return node.map(collectText).join(' ')
  if (typeof node === 'object' && '$rmx' in node) return collectText((node as RemixElement).props.children)
  return ''
}

function findOption(node: RemixNode, value: string): RemixElement | undefined {
  ...
  if (!('$rmx' in node)) return undefined
  let element = node as RemixElement
  if (element.type === ListboxOption && element.props.value === value) return element
...
function SelectedCheckIcon(handle: Handle<{ className?: string }>) {
  return () => (
    <svg
      className={cx('size-3.5 shrink-0 text-primary', handle.props.className)}
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
...
export function Listbox(handle: Handle<ListboxProps>) {
  let currentValue: string | null = handle.props.value ?? handle.props.defaultValue ?? null
  ...
  return () => {
    ...
    let selectedProps = selected ? splitProps(selected.props as ListboxOptionProps) : undefined
    ...
    return (
      <select.Context
        ...
        defaultLabel={nodeText(placeholder)}
        disabled={disabled}
      >
        <span data-slot="control" className="relative block w-full">
          <button
            ...
            mix={[
              ...
              select.onSelectChange((event) => {
                if (event.value === null || event.value === currentValue) return
            ...
            className={cx(
              ...
              'group relative flex w-full cursor-pointer items-center gap-x-2 text-nowrap',
              'rounded-lg border border-layer-line bg-layer text-layer-foreground',
              'py-3 ps-4 pe-9 text-start text-sm',
              'hover:bg-layer-hover focus:bg-layer-focus focus:outline-hidden',
              'focus-visible:ring-2 focus-visible:ring-primary-focus focus-visible:ring-offset-1 focus-visible:ring-offset-background',
              // Invalid
          ...
          >
            <span className="flex min-w-0 flex-1 items-center">
              {selected ? (
                <span className={cx(selectedProps?.className, optionContentClasses)}>{selected.props.children}</span>
              ) : placeholder ? (
...
function ListboxOptions(handle: Handle<{ children?: RemixNode }>) {
...
export function ListboxOption(handle: Handle<ListboxOptionProps>) {
  return () => {
    let { className, rest } = splitProps(handle.props)
    ...
    return (
      <div
        {...divProps}
        mix={[select.option({ value, label: label ?? nodeText(children) ?? value, disabled }), mix]}
        className={cx(
          // Preline select item
      ...
      >
        ...
        <SelectedCheckIcon className="hidden group-aria-selected/option:block" />
      </div>
    )
  }
}

export function ListboxLabel(handle: Handle<ListboxLabelProps>) {
  return () => {
    let { className, rest } = splitProps(handle.props)
...
export function ListboxDescription(handle: Handle<ListboxDescriptionProps>) {
  return () => {
    let { className, rest } = splitProps(handle.props)
    let { children, ...props } = rest
    return (
