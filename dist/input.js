...
export function InputGroup(handle) {
    return () => {
        ...
        return (_jsx("span", { "data-slot": "control", ...attrs, className: cx(className, 'relative block w-full', 
            // Icons are absolutely positioned, vertically centered, and never intercept clicks.
            '[&>[data-slot=icon]]:pointer-events-none [&>[data-slot=icon]]:absolute [&>[data-slot=icon]]:top-1/2 [&>[data-slot=icon]]:z-20 [&>[data-slot=icon]]:size-4 [&>[data-slot=icon]]:shrink-0 [&>[data-slot=icon]]:-translate-y-1/2 [&>[data-slot=icon]]:text-muted-foreground', '[&>[data-slot=icon]:first-child]:start-4 [&>[data-slot=icon]:last-child]:end-4', 
            // Make room for a leading / trailing icon in the input.
            'has-[>[data-slot=icon]:first-child]:[&_input]:ps-11 has-[>[data-slot=icon]:last-child]:[&_input]:pe-11'), children: children }));
    };
}
...
export function Input(handle) {
    return () => {
        ...
        return (_jsx("span", { "data-slot": "control", className: cx(className, 'relative block w-full has-disabled:opacity-100'), children: _jsx("input", { ...attrs, id: id, disabled: disabled || undefined, "aria-invalid": invalid ? 'true' : undefined, "aria-describedby": describedBy, mix: mixins.length ? mixins : undefined, className: cx(inputClasses, 
                // Native date pickers: keep the same height as text inputs.
                attrs.type && dateTypes.includes(attrs.type) && '[&::-webkit-date-and-time-value]:min-h-[1.5em] [&::-webkit-date-and-time-value]:text-start') }) }));
    };
}
